/*------------------------------------------------------------------------------------------------------/
| Program : InspectionCancelBefore.js
| Event   : InspectionCancelBefore
|
| Usage   : Based on the Universal Master Script. Loads default cap information from inspection model 
|
| Client  : San Mateo County
| Action# : N/A
| Created : 30 June 2022 by Tom Grzegorczyk, Avocette Technologies Inc.
| Notes   : 
|
|
/------------------------------------------------------------------------------------------------------*/
...
var inspectionModel = inspectionList.get(0);

capId = inspectionModel.capID;
if (inspectionModel.cap) {
	cap = inspectionModel.cap;
} else {
	var result = aa.cap.getCap(capId);
	if (result.getSuccess() != true) {
		logDebug("Failed to get cap. " + result.errorType + ': ' + result.errorMessage);
	}
	cap = result.getOutput();
}

if (cap && appTypeArray.length < 1) {
