...
| <===========END=Main=Loop================>
/-----------------------------------------------------------------------------------------------------*/
function mainProcessTest(){
	
	//var capModel = aa.cap.
	
	//var capModel = aa.cap.getCap(capId).getOutput().getCapModel();
	//var capModel =  aa.cap.getByAppType("Building","Photovoltaic").getOutput().getCapModel;
...
function mainProcess() {
	...
	for (aps in apsArray){
		...
		var parcelNumber = getParcelCustom(capId);
		if (parcelNumber) {
			//now check to ensure Accela parcel exists in ESRI
			var gisParcel = findParcelInGISCustom(parcelNumber);
				...
				if (gisObjectResult.getSuccess()) {
					...
					for (a1 in fGisObjs) {
						...
						if(fGisObjs[a1].getGisTypeId() == "Active Parcels"){
							valueToSet = getGISBufferInfoCustom(gisService,"LCP_MIDCOAST_PROJECT_AREA",-1,capId,"OBJECTID_1");
							//valueToSet = getGISBufferInfoCustomBatch(gisService,"FEMA Flood Zones",-1,"FLD_ZONE",capId);
							
							
							strValueToDisplay = evaluateResultCustom(valueToSet,"OBJECTID_1");
							//strValueToDisplay = evaluateResultCustom(valueToSet,"FLD_ZONE");
							
							if (strValueToDisplay != "NA") {
								updateParcelAttrCustom("LPC MIDCOAST PROJECT AREA", "Yes", capId);
								//printLine(myAppTypeArray[0] + "/" + myAppTypeArray[1] + " - " + customId + " - " + parcelNumber + " - " + strValueToDisplay);
								//printLine(customId + " - " + parcelNumber + " - " + strValueToDisplay);
								processCount++;
							}
...
/*******************************************************
| Script/Function: updateParcelAttrCustom()
| Created by: Mike Buell
| Created on: 02June17
...
| Modified on: 
*********************************************************/
function updateParcelAttrCustom(attField, theValue, capId) {
		...
		if (primaryParcel == "Y"){
			...
			for (x in apoAtts){
				if(apoAtts[x].getB1AttributeName() == attField){
					//printLine("Initial value - " + apoAtts[x].getB1AttributeName() + " - " + apoAtts[x].getB1AttributeValue());
