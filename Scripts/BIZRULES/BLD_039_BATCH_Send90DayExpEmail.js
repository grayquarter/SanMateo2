/*******************************************************
| Script Title: Batch_Send90DayExpEmail(ID39)
| Created by: Nic Bunting
| Created on: 3Nov20
| Usage: For all Building records If today is 90 days from ASI "Date of Expiration" And task "Inspections" is active Then send notification
| "BLD_PERMITEXP_90DAYWARNING" to all contacts and licensed professionals
| Modified by: ()
*********************************************************/
/****************************************************************************************************************************
...
if (paramsOK) {
    ...
    if (timeExpired == true) {
        ...
        try {
            (function () {
                Avo_LogDebug('ID39 send90DayExpEmail()', 1);

                var ninetyDaysFromNow = Avo_GetToday();
                ...
                if (runDate && runDate.toLowerCase() != "null" && runDate.length > 0 && regex.test(runDate)) {
                    ninetyDaysFromNow = Avo_GetDateFromAccelaDateString(runDate);
                }

                ninetyDaysFromNow.setDate(ninetyDaysFromNow.getDate() + 90);
                var ninetyDaysFromNowStr = aa.util.formatDate(ninetyDaysFromNow, "MM/dd/yyyy");
                Avo_LogDebug("90 Days from Now(" + ninetyDaysFromNowStr + ")", 2);	//debug

                var fieldName = "Date of Expiration";

                var result = aa.cap.getCapIDsByAppSpecificInfoField(fieldName, ninetyDaysFromNowStr);
                if (result.getSuccess() != true) {
                    Avo_LogDebug('Failed to find any records with an Expiration date of ' + ninetyDaysFromNowStr + '. '
                ...
                var template = "BLD_PERMITEXP_90DAYWARNING";
