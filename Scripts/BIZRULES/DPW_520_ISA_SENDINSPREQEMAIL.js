/*******************************************************
| Script/Function: sendInspSchedEmail() - (ID20)
| Created by: Andy Cervanez
| Created on: 
| Usage: When inspection is scheduled in ACA Then send notification "DPW_INSPECTIONREQUESTED" 
...
(function () {
    ...
    var template = "DPW_INSPECTIONREQUESTED";
    ...
    //addParameter(params, "$$inspTime$$", String(inspSchedTime));
    //Avo_LogDebug("Insp Time(" + inspSchedTime + ")", 2);
    ...
    if (vEventName == "InspectionMultipleScheduleAfter" || matches(currentUserID, "IVRUSER") == true) {
        inspectionSchedDate.setDate(inspectionSchedDate.getDate());
    }
...
aa.sendMail("noreply@smcgov.org", "PI_Test@avocette.com", "", "SMC Prod: DPW_520_ISA_SendInspSchedEmail", debug); //debug
