/*******************************************************
| Script Title: Batch_Send180DayPastExpEmail(ID43)
| Created by: Nic Bunting
| Created on: 2Dec20
| Usage: For all Building records If today is 180 days after ASI "Date of Expiration" Then send notification "BLD_PERMITCANCELLED" to
| all "Applicant" contacts, record owner, and licensed professionals
| Modified by: ()
*********************************************************/
/****************************************************************************************************************************
...
| START: TEST DATA
/------------------------------------------------------------------------------------------------------*/
aa.env.setValue("runDate", "11/17/2022");    //testing
...
if (paramsOK) {
    ...
    if (timeExpired == true) {
        ...
        try {
            (function () {
                Avo_LogDebug('ID43 send180DayPastExpEmail()', 1);

                var oneHundredEightyDaysAgo = Avo_GetToday();
                ...
                if (runDate && runDate.toLowerCase() != "null" && runDate.length > 0 && regex.test(runDate)) {
                    oneHundredEightyDaysAgo = Avo_GetDateFromAccelaDateString(runDate);
                }

                oneHundredEightyDaysAgo.setDate(oneHundredEightyDaysAgo.getDate() - 180);

                var oneHundredEightyDaysAgoStr = aa.util.formatDate(oneHundredEightyDaysAgo, "MM/dd/yyyy");
                Avo_LogDebug("180 Days Ago(" + oneHundredEightyDaysAgoStr + ")", 2);	//debug

                var fieldName = "Date of Expiration";

                var result = aa.cap.getCapIDsByAppSpecificInfoField(fieldName, oneHundredEightyDaysAgoStr);
                if (result.getSuccess() != true) {
                    Avo_LogDebug('Failed to find any records with an Expiration date of ' + oneHundredEightyDaysAgoStr + '. '
